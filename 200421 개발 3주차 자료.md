## 200421 개발 3주차 자료
#### 3 주차 목표 : 2주차 이어서 학습 + 크롤링기초/ 1:N Model 연결/ 여러 app 연결/ 코드의 재사용 / Git
...
- 1 ) 'Lion'
먼저 ForeignKey 에는 Post모델과 연결해줄 모델(Lion)을 연결해주고
...
그런데 장고는 인간이 아니라서 그걸 알려줘야 하는데 그게 `on_delete=models.CASCADE` 입니다. <br/>
- 3 ) 'blank'와 'null'
아마 이미 Lion 모델로 DB를 만들어서 admin page에서 객체를 생성해줬을거에요  <br/>
...
따라서 name 이라는 칼럼에 대해서 비어있어도 되고(blank = True), 없어도 된다(null = True)라고 해줘야 <br/>
다음에 모델을 수정하거나 추가할 때에도 문제가 발생하지 
<br/>
