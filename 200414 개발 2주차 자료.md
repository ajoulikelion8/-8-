...
- 2 url-view-templates 의 관계를 이해하고 html에 나타낼 데이터를 처리하는 view를 공부 
    : 환율계산 페이지 실습 (Templates - view - url)
- 3 view에서 처리하는 데이터를 관리하기 위한 DB를 장고에서 처리해보기
    : Django MTV pattern & ORM (Model) / admin page
- 4 DB(model)에서 데이터를 가져와 view에서 처리해준 후 html에 나타내기
...
C언어에서는 구조체(Struct) / JAVA나 python 에서는 Class 를 말한다<br/><br/>
...
<br/><br/>
Django에서도 이 객체를 사용해서 인스턴스들을 DB에 데이터를 저장하는데 <br/>
django는 python 을 사용하니 Class를 통해 객체를 사용해서 인스턴스를 만들어줍니다
<br/><br/>

그래서 이 Class를 작성해주는 곳이 Model이고<br/>
...
<br/><br/>
### MTV pattern
이제 model에서 데이터를 가져와서 view에서 처리를 한 후 Templates에 보여줘야겠죠?<br/>
