...
[난이도 하 버전!] :  실시간 환율 정보 가져오는 거 X, 우리가 직접 입력 & 나라별로 html form, submit버튼, 페이지 결과 페이지 다 다르게 만들기.



![image](https://user-images.githubusercontent.com/42667951/79092239-3917ab00-7d8b-11ea-9b68-d9e9e853b339.png)

```html
<!-- home.html -->
<form action="{% url 'usaresult' %}" method="post" >
    {% csrf_token %}
...
```python
#views.py
def usaresult(request):
    korea = request.POST['korea']
    korea = int(korea)
    usa = korea/1,218.50
    return render(request,'exchangeapp/usaresult.html',{'korea':korea,'usa':usa})
...
urlpatterns = [
  ...
  path('usaresult/',exchangeapp.views.usaresult,name='usaresult')
]
```
