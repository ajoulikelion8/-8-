...
- 1 1주차 리뷰 (django project/ app/ hello world에 대한 이해)
- 2 staticfiles(css 연결)
- 3 url-view-templates 의 관계를 이해하고 html에 나타낼 데이터를 처리하는 view를 공부 <br/>
    : 환율계산 페이지 실습 (Templates - view - url)
- 4 view에서 처리하는 데이터를 관리하기 위한 DB를 장고에서 처리해보기 <br/>
    : Django MTV pattern & ORM (Model) / admin page
- 5 DB(model)에서 데이터를 가져와 view에서 처리해준 후 html에 나타내기 <br/>
...
- Project 만들기 `django-admin startproject myproject`<br/>
프로젝트는 웹 개발의 가장 큰 틀 (사진첨부할것)
- Project 폴더로 가기 `cd myproject`
- app 만들기 `python manage.py startapp myapp`<br/>
앱은 로그인, 메인페이지, 회원가입 등 기능들을 모듈화 해서 관리하기 위함(사진첨부할것)
...
def index(request):
    return render(request, 'index.html') # index.html을 render하게 request 해준다
```
